 ...
 * Port of the portal's src/lib/deal-validation.mjs (zod swapped for the
 * dependency-free schema.mjs). Same two layers:
 *   1. Shape (schema.mjs) — friendly, field-pathed messages.
 *   2. Soundness (the real engine) — equal arrays, finite IRR, DSCR, a teaser
 *      headline that actually matches the model.
 *
 * Returns { ok, errors: string[], warnings: string[], computed: {...} | null }.
 * Pure, no I/O — callers load the JSON.
 */
import { run } from './engine.mjs';
import { validateShape } from './schema.mjs';

const BASE_HOLD = 7; // the interactive ModelIsland opens at a 7-yr hold; validate against the same.
...
export function validateDeal(deal) {
  ...
  // ── Layer 1: shape ─────────────────────────────────────────────────────────
  const issues = validateShape(deal);
  if (issues.length) {
    for (const it of issues) errors.push(`${it.path}: ${it.message}`);
