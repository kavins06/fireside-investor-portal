...
5. **Build the single HTML file** to the brand + component contract (Sections 5–7)
   with the LP/GP behavior in Section 6.
6. **Save** to the user's Fireside folder as
   `<Deal Name> — Investor Dashboard.html` and present it.
7. **Summarize assumptions.** After delivering, list — briefly — every value you
...
8. **Learn.** Record durable facts/preferences to memory (Section 9).
...
// IRR by bisection
function irr(cfs){let lo=-0.9,hi=5,npv=r=>cfs.reduce((s,c,t)=>s+c/Math.pow(1+r,t),0);
